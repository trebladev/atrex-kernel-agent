...
1. Read only the conversion sheet matching the authoritative runtime architecture:
   ...
   - `sm_110`: `gpu-wiki/docs/nvidia/blackwell/converter/blackwell.md`, but treat its
     tcgen05/TMEM constructs as unverified on Thor: first check
     `gpu-wiki/docs/nvidia/blackwell-thor/hardware-specs/hardware_specs_sm110.md` and probe
     them on-device before relying on them
